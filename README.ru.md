...
Русские Markdown-источники находятся в `docs/`; соответствующие английские
Markdown-источники находятся в `docs/en/` с теми же относительными путями.
Во время публикации `docs/build-pages.js` рендерит эти парные источники в
русскую и английскую HTML-оболочку с локальной навигацией, связанными
документами и ссылками внутри той же языковой версии. Публикация идет из
директории `docs` через GitHub Actions в ветку `gh-pages`. В настройках Pages
нужно выбрать source `Deploy from a branch`, branch `gh-pages`, folder
`/ (root)`.
