...
| `POST /api/modules/reload` | `keys?`, `module_name?`, `reason?` | `config_reload.result.v1` or `module_reload.result.v1` | Stage 02 synchronous operation. Without `module_name`, reloads config keys and applies `modules.enabled` to persisted `module_states`; with `module_name`, reloads one available module. Stage 03 may add job-backed `config_reload` handlers for long-running workflow integration. |
| `POST /api/modules/restart` | `module_name`, `reason?` | `module_restart.result.v1` | Stage 02 synchronous restart for one available module; unavailable modules return controlled `module_unavailable`. Stage 03 may add job-backed `module_restart` handlers for worker/status integration. |
